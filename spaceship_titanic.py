...
report_df = pd.DataFrame({
    'Classe': classes,
    'Precisão': [report[classe]['precision'] * 100 for classe in classes],
...
plt.title('Classification Report (em Percentual)')
plt.xlabel('Classes')
plt.ylabel('Métricas (%)')
...
plt.title('Distribuição da Idade por Status de Transporte')
plt.xlabel('Idade')
plt.ylabel('Densidade')
plt.legend(title='Transportado', labels=['Não', 'Sim'])
plt.show()
...
sns.heatmap(cm_reordered, annot=True, fmt='d', cmap='Blues', 
            yticklabels=['Verdadeiros', 'Falsos'], 
